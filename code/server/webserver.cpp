...
WebServer::WebServer(
    ...
    // /home/cx/projects/MyWebServer
    srcDir_ = getcwd(nullptr, 256); // 获取当前的工作路径
    assert(srcDir_);

    // /home/cx/projects/MyWebServer/resources/
    strncat(srcDir_, "/resources/", 16);    // 拼接资源路径，这个路径是服务器资源的根路径
...
void WebServer::InitEventMode_(int trigMode) {
    ...
    switch (trigMode) {
        case 0:
            break;
        case 1:
            connEvent_ |= EPOLLET;      // trigMode 为 1 表示：通信的文件描述符的模式为 ET 模式
            break;
        case 2:
            listenEvent_ |= EPOLLET;    // trigMode 为 2 表示：监听的文件描述符的模式为 ET 模式
            break;
        case 3:
            listenEvent_ |= EPOLLET;    
            connEvent_ |= EPOLLET;      // trigMode 为 3 表示：监听的文件描述符 和 通信的文件描述符的模式都为 ET 模式
            break;
        default:
            listenEvent_ |= EPOLLET;
            connEvent_ |= EPOLLET;      // 默认情况 表示：监听的文件描述符 和 通信的文件描述符的模式都为 ET 模式
            break;
    }
...
void WebServer::Start() {
    ...
    // 只要服务端不关闭，就一直检测 epoll 事件是否有数据到达
    while(!isClose_) {
...
bool WebServer::InitSocket_() {
    ...
    // 将监听文件描述符添加到 epoll 事件中
    // 检测 listenFd_ 是否有数据到达，有数据到达表示有客户端连接进来
