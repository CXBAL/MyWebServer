...
public:
    WebServer(
        int port, int trigMode, int timeoutMS, bool OptLinger,  // 端口 ET模式  timeoutMS 是否优雅退出   
        int sqlPort, const char* sqlUser, const  char* sqlPwd,  // MySQL端口 MySQL用户名 MySQL密码
        const char* dbName, int connPoolNum, int threadNum,     // MySQL数据库名 连接池数量 线程池数量
...
private:
    ...
    std::unordered_map<int, HttpConn> users_;   // 保存的是客户端连接的信息，键：文件描述符；值：对应的客户端信息
};
